...
class PlaywrightManager:
    ...
    async def start(self) -> None:
        ...
        if self._launcher is None:
            ...
            self._playwright = await self._launcher().start()
        ...
        self._started = True
        self.telemetry.record(TimelineEventType.MARK, "browser_started", profile=self.profile.name)
    ...
    async def attach_cdp(self, page: Any) -> None:
        ...
        session.on("Network.requestWillBeSent", self._cdp_tracker.on_request_will_be_sent)
        ...
        * `page=<單頁>` -> **不得** clear：tracker 為 manager 範圍共用，其他仍掛載的 page
        ...
        if page is not None:
            ...
            if session is not None:
                try:
                    session.remove_listener("Network.requestWillBeSent", self._cdp_tracker.on_request_will_be_sent)
                    session.remove_listener("Network.responseReceived", self._cdp_tracker.on_response_received)
                except Exception:
                    pass
                try:
            ...
            for s in sessions:
                try:
                    ...
                    s.remove_listener("Network.responseReceived", self._cdp_tracker.on_response_received)
                except Exception:
                    pass
                try:
                    await s.detach()
    ...
    async def capture_screenshot(
        ...
        *,
        ...
        if self._closing or not self._started:
            self.telemetry.record(TimelineEventType.MARK, "screenshot_skipped", reason="closing_or_stopped")
            return None
        self.screenshot_dir.mkdir(parents=True, exist_ok=True)
        ...
        def hook(source: str, target: str, event: str) -> None:
            ...
            if self._closing:
                ...
                self.telemetry.record(
                    TimelineEventType.MARK, "screenshot_skipped", reason="closing",
                )
                return
            ...
            if loop is None or loop.is_closed():
                self.telemetry.record(
                    TimelineEventType.MARK, "screenshot_skipped", reason="no_loop",
                )
                return

            frozen_seq = int(sequence)
            frozen_state = str(getattr(target, "id", target))
            frozen_exp = str(experiment_id)

            async def _bg(seq: int = frozen_seq, state_name: str = frozen_state, exp_id: str = frozen_exp) -> None:
                try:
                    await self.capture_screenshot(seq, state_name, page, experiment_id=exp_id)
                except Exception as exc:
                    self.telemetry.record_error("screenshot_failed", exc, state=state_name, experiment_id=exp_id)
            ...
            if running is loop:
                ...
                try:
                    ...
                    self.telemetry.record(
                        TimelineEventType.MARK, "screenshot_skipped", reason="loop_closed",
                    )
